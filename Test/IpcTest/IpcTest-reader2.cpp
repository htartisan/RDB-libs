...
int appMain(int argc, char *argv[])
{
    ...
    auto status = g_globals.appInit("ipcTest-reader2 (IPC primary)", "1.0");
    ...
    LogInfo("Opening IPC input channel: {}, MaxBlkSize: {}, Primary: true ", IPC_TEST_NAME, BUFFER_SIZE_IN_BYTES);

    CIpcReader  reader(IPC_TEST_NAME, BUFFER_SIZE_IN_BYTES, true);

    if (reader.open() == false)
    {
        LogError("IPC reader open failed - exiting");
        LogError("CIpcReader 'lastError' = {}", reader.getLastErrorStr());
        return -5;
    }
    ...
    while (bExitApp == false)
    {
        ...
        while (true)
        {
            ...
            if (reader.isWriterConnected() == true)
            {
                break;
            }
