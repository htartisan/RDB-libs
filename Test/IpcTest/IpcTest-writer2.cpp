...
int appMain(int argc, char *argv[])
{
    ...
    auto status = g_globals.appInit("ipcTest-writer2 (IPC secondary)", "1.0");
    ...
    LogInfo("Opening IPC output channel: {}, MaxBlkSize: {}, Primary: false ", IPC_TEST_NAME, BUFFER_SIZE_IN_BYTES);

    CIpcWriter  writer(IPC_TEST_NAME, BUFFER_SIZE_IN_BYTES, false);

    //threadData.setWriter(&writer);
    ...
    while (bExitApp == false)
    {
        ...
        bool bWriterConnected = writer.isReaderConnected();
        while (bWriterConnected == false && bExitApp == false)
        {
            ...
            bWriterConnected = writer.isReaderConnected();
        }
